...
subtitle: "TALLER"
author: 
  - name: Julian Marquez <br>Alex Gomez <br> Rubiel Lopez
    affiliation: "Universidad del Tolima"
    affiliation-url: "https://www.ut.edu.co/"
date: today
lang: es
title-block-banner: "#ed989b"
...
Las variables aleatorias y las distribuciones de probabilidad son conceptos fundamentales dentro de la estadística, ya que permiten modelar y analizar fenómenos en los que interviene el azar. A través de estas herramientas es posible describir el comportamiento de los datos, así como calcular e interpretar probabilidades asociadas a distintos eventos.

Las variables aleatorias pueden ser discretas o continuas, dependiendo de los valores que puedan tomar, y cada una se estudia mediante funciones que representan su comportamiento probabilístico. Estas funciones permiten comprender cómo se distribuyen los valores y facilitan la toma de decisiones en situaciones de incertidumbre.

En este taller se desarrollan ejercicios orientados a la interpretación de eventos, el uso de funciones de distribución y el cálculo de probabilidades, con el fin de fortalecer la comprensión de estos conceptos y su aplicación en diferentes contextos.

# Ejercicio

```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 1.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 2.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 3.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ejemplo 4.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 5.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 6.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 7.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 8.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 9.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 10.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 11.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 14.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/R Estadistica 2/ej 15.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 16.png")
```
...
res_final <- data.frame(
  Ejercicio = c("14. Alimento < 400h", "14. Mediana Alimento", "15. Perro > 15 años | 10", "16. Radiación > 300kcal"),
  Formula_Clave = c("\u222b(200 a 400) f(x)dx", "F(M) = 0.5", "S(15) / S(10)", "\u222b(3 a 6) f(x)dx"),
  Resultado = c(round(p_14a, 4), round(mediana_14, 2), round(p_15d, 4), round(p_16a, 4))
)

res_final %>% kable(caption = "Resultados: Supervivencia y Fenómenos Naturales") %>% kable_classic(full_width = F)
```

# Ejercicio

```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 17.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/R Estadistica 2/ej 18.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 19.png")
```
...
```{r}
knitr::include_graphics("H:/TALLER/Variables Aleatorias/R Estadistica 2/ej 20.png")
```
...
```{r}
res_ej20 <- data.frame(
  Parametro = c("Esperanza E[C]", "Desviación Típica \u03c3_C", "Límite Inferior (75%)", "Límite Superior (75%)"),
  Formula_Aplicada = c("3 \u00b7 E[X] + 50", "3 \u00b7 \u03c3_X", "E[C] - 2\u03c3_C", "E[C] + 2\u03c3_C"),
  Valor_Numerico = c(esp_c, round(sd_c, 4), round(lim_inf, 4), round(lim_sup, 4))
)

res_ej20 %>% 
  kable(caption = "Ejercicio 20: Análisis de Costos de Fumigación", align = "clc") %>% 
  kable_classic(full_width = F) %>%
  row_spec(1, bold = T, color = "darkgreen")
...
```{r}
ggplot(data.frame(x = c(esp_c - 20, esp_c + 20)), aes(x)) +
  # Área de Chebyshev
  annotate("rect", xmin = lim_inf, xmax = lim_sup, ymin = 0, ymax = 1, alpha = 0.2, fill = "orange") +
  geom_vline(xintercept = esp_c, color = "red", linetype = "dashed", size = 1) +
  annotate("text", x = esp_c, y = 0.5, label = "Media = 80", color = "red", angle = 90, vjust = -1) +
  labs(title = "Intervalo de Costos por Hectárea (Chebyshev)",
       subtitle = "Al menos el 75% de los costos caen en la zona naranja",
       x = "Costo Total (ptas)", y = "") +
  theme_minimal() +
